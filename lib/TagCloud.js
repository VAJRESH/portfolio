class TagCloud {
  ...
  constructor(container = document.body, texts, options) {
    ...
    self.texts = texts || [];
    self.textShades = options?.shades || {};
  ...
  _createTextItem(text, index = 0) {
    ...
    if (self.config.useItemInlineStyles) {
      ...
      itemEl.style.color = self?.textColor;
  ...
  _next() {
    ...
    self.items.forEach((item) => {
      ...
      itemEl.style.color = self?.textShades?.back;
      if (alpha > 0.7) itemEl.style.color = self?.textShades?.sideways;
      if (alpha > 0.9) itemEl.style.color = self?.textShades?.front;
    });
  }
...
export const TextSphere = (els, texts, options) => {
